  ...
  event.respondWith(
    caches.match(event.request).then(response => {
      ...
      return fetch(event.request).then(response => {
        ...
        // If fetching from network fails, try fetching from cache using the full URL
        if (event.request.url === 'https://book.earthen.io/en/imagine.php') {
          return caches.match('/en/imagine.php');
        }
